...
SyclVec3 to_sycl_vec3(const Vec3& v) {
    return SyclVec3{v.getX(), v.getY(), v.getZ()};
}
        ...
        q.submit([&](sycl::handler& h) {
            ...
            h.parallel_for(sycl::range<1>(total_points), [=](sycl::id<1> idx) {
                ...
                for (size_t r = 0; r < refs_acc.get_range()[0]; ++r) {
                    ...
                    double dy = p.y - ref.y;
                    double dz = p.z - ref.z;
                    double dist = sycl::sqrt(dx * dx + dy * dy + dz * dz);

                    double term_1 = 0.5 * sycl::exp(-0.1 * dist);
                    double term_2 = sycl::pow(dist, 0.33);
                    double term_3 = 0.25 * dist * dist * dist - 2.5 * dist * dist - 0.3 * dist + 3.2;
                    double term_4 = 0.22 * sycl::pow(dist, -1.5);
                    double term_5 = 0.1 * sycl::pow(dist, -2.5);
                    double term_6 = -0.2 * sycl::pow(dist, -3.5);
                    double term_7 = -0.8 * sycl::pow(dist, 0.5);
