...
int main() 
{
    ...
    // Allocate and copy to raw arrays
    int num_points = points.size();
    int num_refs = reference_points.size();

    Vec3* points_arr = points.data();
    Vec3* ref_arr = reference_points.data();
    double* calc_arr = calculated_results.data();

    #pragma omp target data map(to: points_arr[0:num_points], ref_arr[0:num_refs]) \
                            map(from: calc_arr[0:num_points])
    {
        #pragma omp target teams distribute parallel for
        for (int idx = 0; idx < num_points; ++idx) {
            Vec3 point = points_arr[idx];
            double result_local = 0.0;

            for (int j = 0; j < num_refs; ++j) {
                Vec3 ref = ref_arr[j];
                double dx = point.getX() - ref.getX();
                double dy = point.getY() - ref.getY();
                double dz = point.getZ() - ref.getZ();
