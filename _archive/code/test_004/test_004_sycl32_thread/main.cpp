...
int main() {
    ...
    int mid = total_points * (95 / 100);
    sycl::queue q;

    // GPU kernel for first half
    {
        sycl::buffer<SyclVec3, 1> points_buf(points_device.data(), sycl::range<1>(mid));
        sycl::buffer<SyclVec3, 1> refs_buf(reference_points_device.data(), sycl::range<1>(reference_points_device.size()));
        sycl::buffer<double, 1> result_buf(calculated_results.data(), sycl::range<1>(mid));
        ...
        q.submit([&](sycl::handler& h) {
            ...
            h.parallel_for(sycl::range<1>(mid), [=](sycl::id<1> idx) {
                float acc = 0.0f;
    ...
    // CPU loop for second half (runs in parallel thread)
    std::thread cpu_thread([&]() {
        for (int idx = mid; idx < total_points; ++idx) {
