...
struct Grid {
    ...
    std::vector<__float128> u;
    std::vector<__float128> un;
     ...
     */
    Grid(std::size_t nx_in, std::size_t ny_in, __float128 lx, __float128 ly)
     ...
     */
    inline __float128& at(std::size_t i, std::size_t j) {
        return u[id(i, j)];
    }
     ...
     */
    [[nodiscard]]
    inline __float128 at(std::size_t i, std::size_t j) const {
        return u[id(i, j)];
    }
     ...
     * @warning No bounds checking is performed.
     */
    inline __float128& nxt(std::size_t i, std::size_t j) {
        return un[id(i, j)];
    }
     ...
     */
    [[nodiscard]]
    inline __float128 a(std::size_t i, std::size_t j) const {
        return alpha[id(i, j)];
    }
    ...
    static Grid Load_settings(const nlohmann::json& config_file) {
        ...
        const auto length_x{config_file.at("length_x").get<__float128>()};
        const auto length_y{config_file.at("length_y").get<__float128>()};
     ...
     */
    static void grid_to_csv(const std::filesystem::path& fname, const Grid& model_grid, __float128 t) {
...
static void set_alpha_regions(Grid& model_grid, const nlohmann::json& jalpha) {
    ...
    const auto base{jalpha["value"].get<__float128>()};
    if (!(base > 0.0)) {
    ...
    for (auto r{std::size_t(0)}; r < customs.size(); r++) {
        ...
        const auto x_min{rect["x_min"].get<__float128>()};
        const auto x_max{rect["x_max"].get<__float128>()};
        const auto y_min{rect["y_min"].get<__float128>()};
        const auto y_max{rect["y_max"].get<__float128>()};
        const auto aval{rect["value"].get<__float128>()};
