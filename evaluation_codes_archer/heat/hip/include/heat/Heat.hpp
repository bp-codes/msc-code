...
/**
 * @file Heat.hpp
 * @brief Heat equation solver driver using a HIP backend.
 *
 * Reads a JSON configuration, initializes the simulation grid and sources,
 * sets up a HIP engine (device buffers), advances the solution in time,
...
public:
     ...
     * a HIP engine for the selected device, runs the solver, and prints execution time.
     *
    ...
    static void run(std::string& input_file) {
        ...
        // Read input
        Heat::read_input(input_file, device, model_grid, sources, dt, t_final, snapshot_every,
                         outdir, prefix, queue_output);
        ...
        // Set up hip engine (will store grid and sources on device)
        HipEngine hip_engine{device, model_grid.nx, model_grid.ny, sources.size()};

        // Heat grid
        Heat::heat_grid_hip(hip_engine, model_grid, sources, dt, t_final, snapshot_every, outdir,
                            prefix, queue_output);
...
private:
     ...
     * @throws nlohmann::json::exception If required JSON fields are missing or have the wrong type.
     */
    static void read_input(std::string& input_file, std::string& device, Grid& model_grid,
                           std::vector<Source>& sources, double& dt, double& t_final,
                           int& snapshot_every, std::filesystem::path& outdir,
                           std::filesystem::path& prefix, bool& queue_output) {
        ...
        if (t_final <= 0.0)
            throw std::runtime_error("t_final must be > 0");
    ...
    /**
     * @brief Advance the heat equation in time using the HIP backend and write snapshots.
     *
     ...
     * the grid for output.
     *
     * @param hip_engine HIP execution engine holding device buffers.
     * @param model_grid Grid state (updated in-place; downloaded from device when output is
     * needed).
     ...
     */
    static void heat_grid_hip(HipEngine& hip_engine, Grid& model_grid,
                              const std::vector<Source>& sources, const double& dt,
                               const double& t_final, const int& snapshot_every,
                               const std::filesystem::path& outdir,
                               const std::filesystem::path& prefix, const bool queue_output) {
        ...
        hip_engine.upload_grid(model_grid, sources);
        ...
        while (t < t_final - 1e-15) {
            ...
            hip_engine.heat_step(dt, t_sample);

            // Zero boundaries
            hip_engine.dirichlet_boundaries();

            // ---- swap device buffers for next step
            hip_engine.swap_buffers();
            ...
            if (step % snapshot_every == 0 || t >= t_final - 1e-15) {
                // Copy u from device
                hip_engine.download_grid(model_grid);
