...
struct Grid {
     ...
     */
    Grid(std::size_t nx_in, std::size_t ny_in, float lx, float ly)
        ...
        alpha.assign(nx * ny, 0.0f);
        u.assign(nx * ny, 0.0f);
        un.assign(nx * ny, 0.0f);
    }
     ...
     */
    inline float& at(std::size_t i, std::size_t j) {
        return u[id(i, j)];
    }
     ...
     */
    [[nodiscard]]
    inline float at(std::size_t i, std::size_t j) const {
        return u[id(i, j)];
    }
     ...
     * @warning No bounds checking is performed.
     */
    inline float& nxt(std::size_t i, std::size_t j) {
        return un[id(i, j)];
    }
     ...
     */
    [[nodiscard]]
    inline float a(std::size_t i, std::size_t j) const {
        return alpha[id(i, j)];
    }
    ...
    static Grid Load_settings(const nlohmann::json& config_file) {
        ...
        const auto length_x{config_file.at("length_x").get<float>()};
        const auto length_y{config_file.at("length_y").get<float>()};
     ...
     *
     * Sets u(i,0), u(i,ny-1), u(0,j), u(nx-1,j) to zero for all boundary indices.
     *
    ...
    static void dirichlet_boundaries(Grid& model_grid) {
        ...
        for (auto i{std::size_t(0)}; i < model_grid.nx; i++) {
            model_grid.at(i, 0) = 0.0f;
            model_grid.at(i, model_grid.ny - 1) = 0.0f;
        }
        for (auto j{std::size_t(0)}; j < model_grid.ny; j++) {
            model_grid.at(0, j) = 0.0f;
            model_grid.at(model_grid.nx - 1, j) = 0.0f;
        }
    }
     ...
     *
     * Writes a header line with time and metadata, followed by ny rows of nx values.
     *
     ...
     */
    static void grid_to_csv(const std::filesystem::path& fname, const Grid& model_grid, float t) {
 ...
 *
 * Initializes all alpha values to alpha.value, then optionally overwrites rectangular
 * regions described in alpha.custom.
 *
 * Expected schema:
 * - alpha: { "value": <number>, "custom": [ {x_min,x_max,y_min,y_max,value}, ... ] }
 *
 * Rectangles are clamped to the physical domain [0,length_x] x [0,length_y].
 *
 * @param model_grid Grid whose alpha field will be modified.
 * @param jalpha JSON object describing alpha configuration.
 *
 * @throws std::runtime_error If required fields are missing/invalid or if alpha values are <= 0.
...
static void set_alpha_regions(Grid& model_grid, const nlohmann::json& jalpha) {
    ...
    const auto base{jalpha["value"].get<float>()};
    if (!(base > 0.0f)) {
    ...
    for (auto r{std::size_t(0)}; r < customs.size(); r++) {
        ...
        const auto x_min{rect["x_min"].get<float>()};
        const auto x_max{rect["x_max"].get<float>()};
        const auto y_min{rect["y_min"].get<float>()};
        const auto y_max{rect["y_max"].get<float>()};
        const auto aval{rect["value"].get<float>()};
        ...
        const auto xminc{std::max(0.0f, std::min(x_min, model_grid.length_x))};
        const auto xmaxc{std::max(0.0f, std::min(x_max, model_grid.length_x))};
        const auto yminc{std::max(0.0f, std::min(y_min, model_grid.length_y))};
        const auto ymaxc{std::max(0.0f, std::min(y_max, model_grid.length_y))};
