...
public:
    ...
    static void run(std::string& input_file) {
        ...
        // Read input
        Heat::read_input(input_file, model_grid, sources, dt, t_final, snapshot_every, outdir,
                         prefix, queue_output);
        ...
        // Heat grid
        Heat::heat_grid(model_grid, sources, dt, t_final, snapshot_every, outdir, prefix,
                        queue_output);
...
private:
     ...
     * @throws nlohmann::json::exception If required JSON fields are missing or have the wrong type.
     */
    static void read_input(std::string& input_file, Grid& model_grid, std::vector<Source>& sources,
                           float& dt, float& t_final, int& snapshot_every,
                           std::filesystem::path& outdir, std::string& prefix, bool& queue_output) {
     ...
     */
    static void heat_grid(Grid& model_grid, const std::vector<Source>& sources, const float& dt,
                          const float& t_final, const int& snapshot_every,
                          const std::filesystem::path& outdir, const std::filesystem::path& prefix,
        ...
        while (t < t_final - 1.0e-15f) {
            ...
            for (auto j{std::size_t(1)}; j < model_grid.ny - 1; j++) {
                for (auto i{std::size_t(1)}; i < model_grid.nx - 1; i++) {
                    ...
                    const auto lap =
                        (model_grid.at(i + 1, j) - 2.0f * uij + model_grid.at(i - 1, j)) *
                            model_grid.invdx2 +
                        (model_grid.at(i, j + 1) - 2.0f * uij + model_grid.at(i, j - 1)) *
                            model_grid.invdy2;

                    // Add up contributions from sources S(x,y,t)
                    auto source_accumulator{0.0f};
                    auto constant{-1.0f};
                    for (const auto& s : sources) {
                        const auto this_source{
                            source_value_at(s, t_sample, x, y, dt, model_grid.dx, model_grid.dy)};
