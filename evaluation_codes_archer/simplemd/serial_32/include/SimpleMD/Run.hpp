...
public:
    ...
    static void run(const std::filesystem::path& input_file)
    {
        ...
        ConfigurationEngine::perturb(configuration.get_crystal_size(), 0.01f, configuration);
    ...
    static void load_json(const std::filesystem::path& input_file, Configuration& configuration)
    {
        ...
        auto alat {Run::load<float>(config, {"crystal", "alat"})};
        auto n {Run::load<std::size_t>(config, {"crystal", "size"})};
        auto ux {Run::load<std::vector<float>>(config, {"crystal", "ux"})};
        auto uy {Run::load<std::vector<float>>(config, {"crystal", "uy"})};
        auto uz {Run::load<std::vector<float>>(config, {"crystal", "uz"})};

        auto r_cutoff {Run::load<float>(config, {"simulation", "r_cutoff"})};
        auto r_verlet_cutoff {Run::load<float>(config, {"simulation", "r_verlet_cutoff"})};
        auto rebuild_every {Run::load<std::size_t>(config, {"simulation", "rebuild_every"})};
        auto dt {Run::load<float>(config, {"simulation", "dt"})};
