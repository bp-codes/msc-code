...
[[nodiscard]]
inline std::string random_suffix(const std::size_t n) {
 ...
 */
[[nodiscard]]
inline double parse_floating_point(const char* s) {
    if (s == nullptr) {
 ...
 */
template <typename T>
inline void validate_sizes(const std::vector<T>& numbers_a, const std::vector<T>& numbers_b,
...
template <typename T>
[[nodiscard]]
inline std::string to_string_precise(const T& value) {
...
template <typename T>
[[nodiscard]]
inline T check_sum(const std::vector<T>& numbers) {
