...
TEST(Vec3Length, ThreeFourTwelveGivesThirteen) {
    ...
    const float found{v.length()};
    const float expected{13.0};
    const float tolerance{1.0e-9};
    EXPECT_NEAR(found, expected, tolerance);
}

// ---- Sign invariance & decimals ----
...
TEST(Vec3Length, DecimalComponents) {
    ...
    const float found{v.length()};
    const float expected{3.535533906};  // precomputed
    const float tolerance{1.0e-9};
    EXPECT_NEAR(found, expected, tolerance);
}
...
TEST(Vec3Length, WorksWithLargeMagnitudes) {
    ...
    const float found{v.length()};
    const float expected{1.732050807568877e150};  // A * sqrt(3)
    const float tolerance{1.0e141};               // relative ~1e-9
    EXPECT_NEAR(found, expected, tolerance);
}
