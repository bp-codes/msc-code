...
def plot_difference_histograms(
    ...
    for method, metrics in sorted(grouped.items()):
        ...
        summary.append([safe_method, mean, std])
    ...
    file_name = f"{trial}_{selected_operation}.csv".replace(" ", "_")
    fh = open(analysis_dir / file_name, "w")
    for line in summary:
        fh.write(str(line[0]) + "," + str(line[1]) + "," + str(line[2]) + "\n")
    fh.close()
