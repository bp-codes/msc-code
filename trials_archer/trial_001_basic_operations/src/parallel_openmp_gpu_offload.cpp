 ...
 */
void parallel_add(const double* numbers_a, const double* numbers_b, double* numbers_c,
                  const std::size_t n) {
#pragma omp target teams distribute parallel for \
    num_teams(1024) thread_limit(256) \
    is_device_ptr(numbers_a, numbers_b, numbers_c)
 ...
 */
void parallel_multiply(const double* numbers_a, const double* numbers_b, double* numbers_c,
                       const std::size_t n) {
#pragma omp target teams distribute parallel for is_device_ptr(numbers_a, numbers_b, numbers_c)
    for (auto i = std::size_t(0); i < n; i++) {
        numbers_c[i] = numbers_a[i] * numbers_b[i];
    }
}
 ...
 */
void parallel_divide(const double* numbers_a, const double* numbers_b, double* numbers_c,
                     const std::size_t n) {
#pragma omp target teams distribute parallel for is_device_ptr(numbers_a, numbers_b, numbers_c)
    for (auto i = std::size_t(0); i < n; i++) {
 ...
 */
void parallel_power(const double* numbers_a, const double* numbers_b, double* numbers_c,
                    const std::size_t n) {
#pragma omp target teams distribute parallel for is_device_ptr(numbers_a, numbers_b, numbers_c)
    for (auto i = std::size_t(0); i < n; i++) {
 ...
 */
void parallel_exp(const double* numbers_a, const double* numbers_b, double* numbers_c,
                  const std::size_t n) {
#pragma omp target teams distribute parallel for is_device_ptr(numbers_a, numbers_b, numbers_c)
    for (auto i = std::size_t(0); i < n; i++) {
 ...
 */
void parallel_log(const double* numbers_a, const double* numbers_b, double* numbers_c,
                  const std::size_t n) {
#pragma omp target teams distribute parallel for is_device_ptr(numbers_a, numbers_b, numbers_c)
    for (auto i = std::size_t(0); i < n; i++) {
 ...
 * @warning Inputs must be >= 0. No bounds/validity checking is performed in this hot loop.
 */
void parallel_sqrt(const double* numbers_a, const double* numbers_b, double* numbers_c,
                   const std::size_t n) {
#pragma omp target teams distribute parallel for is_device_ptr(numbers_a, numbers_b, numbers_c)
    for (auto i = std::size_t(0); i < n; i++) {
 ...
 * @param numbers_a First input vector on device.
 * @param numbers_b Second input vector on device.
 * @param numbers_c Output vector on device.
 * @param n Number of elements.
 */
void parallel_task(OperationKind operation, const double* numbers_a, const double* numbers_b,
                   double* numbers_c, const std::size_t n) {
    switch (operation) {
        case OperationKind::Add: {
            parallel_add(numbers_a, numbers_b, numbers_c, n);
            return;
        }
        case OperationKind::Multiply: {
            parallel_multiply(numbers_a, numbers_b, numbers_c, n);
            return;
        }
        case OperationKind::Divide: {
            parallel_divide(numbers_a, numbers_b, numbers_c, n);
            return;
        }
        case OperationKind::Power: {
            parallel_power(numbers_a, numbers_b, numbers_c, n);
            return;
        }
        case OperationKind::Exp: {
            parallel_exp(numbers_a, numbers_b, numbers_c, n);
            return;
        }
        case OperationKind::Log: {
            parallel_log(numbers_a, numbers_b, numbers_c, n);
            return;
        }
        case OperationKind::Sqrt: {
            parallel_sqrt(numbers_a, numbers_b, numbers_c, n);
            return;
        }
    }
...
auto main(int argc, char** argv) -> int {
    ...
    try {
        ...
        if (num_devices <= 0) {
            THROW_RUNTIME_ERROR("No OpenMP target devices found.");
        }
        ...
        const auto* numbers_a_ptr{numbers_a.data()};
        const auto* numbers_b_ptr{numbers_b.data()};
        auto* numbers_c_ptr{numbers_c.data()};


#pragma omp target data map(to: numbers_a_ptr[0:n], numbers_b_ptr[0:n]) \
    map(from: numbers_c_ptr[0:n]) use_device_ptr(numbers_a_ptr, numbers_b_ptr, numbers_c_ptr)
        {
            do {
                parallel_task(operation, numbers_a_ptr, numbers_b_ptr, numbers_c_ptr, n);
                iters++;
        ...
        const auto method{std::string("OpenMP GPU Offload")};
        const auto comments{std::string("operation:") + std::string(operation_string)};

        // Output
        {
            const std::string base_file_name =
                "results/parallel_openmp_gpu_offload_" + std::string(operation_string);
