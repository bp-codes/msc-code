 ...
 */
void parallel_add(const float* numbers_a, const float* numbers_b, float* numbers_c,
                  const std::size_t n) {
 ...
 */
void parallel_multiply(const float* numbers_a, const float* numbers_b, float* numbers_c,
                       const std::size_t n) {
#pragma omp target teams distribute parallel for is_device_ptr(numbers_a, numbers_b, numbers_c)
    for (auto i = std::size_t(0); i < n; i++) {
        numbers_c[i] = numbers_a[i] * numbers_b[i];
 ...
 */
void parallel_divide(const float* numbers_a, const float* numbers_b, float* numbers_c,
                     const std::size_t n) {
#pragma omp target teams distribute parallel for is_device_ptr(numbers_a, numbers_b, numbers_c)
    for (auto i = std::size_t(0); i < n; i++) {
 ...
 */
void parallel_power(const float* numbers_a, const float* numbers_b, float* numbers_c,
                    const std::size_t n) {
#pragma omp target teams distribute parallel for is_device_ptr(numbers_a, numbers_b, numbers_c)
    for (auto i = std::size_t(0); i < n; i++) {
 ...
 */
void parallel_exp(const float* numbers_a, const float* numbers_b, float* numbers_c,
                  const std::size_t n) {
#pragma omp target teams distribute parallel for is_device_ptr(numbers_a, numbers_b, numbers_c)
    for (auto i = std::size_t(0); i < n; i++) {
 ...
 */
void parallel_log(const float* numbers_a, const float* numbers_b, float* numbers_c,
                  const std::size_t n) {
#pragma omp target teams distribute parallel for is_device_ptr(numbers_a, numbers_b, numbers_c)
    for (auto i = std::size_t(0); i < n; i++) {
 ...
 * @warning Inputs must be >= 0. No bounds/validity checking is performed in this hot loop.
 */
void parallel_sqrt(const float* numbers_a, const float* numbers_b, float* numbers_c,
                   const std::size_t n) {
#pragma omp target teams distribute parallel for is_device_ptr(numbers_a, numbers_b, numbers_c)
    for (auto i = std::size_t(0); i < n; i++) {
 ...
 */
void parallel_task(OperationKind operation, const float* numbers_a, const float* numbers_b,
                   float* numbers_c, const std::size_t n) {
    switch (operation) {
...
#pragma omp target data map(to: numbers_a_ptr[0:n], numbers_b_ptr[0:n]) \
    map(from: numbers_c_ptr[0:n]) use_device_ptr(numbers_a_ptr, numbers_b_ptr, numbers_c_ptr)
        ...
        const auto method{std::string("OpenMP GPU Offload 32")};
        const auto comments{std::string("operation:") + std::string(operation_string)};

        // Output
        {
            const std::string base_file_name =
                "results/parallel_openmp_gpu_offload__32_" + std::string(operation_string);
