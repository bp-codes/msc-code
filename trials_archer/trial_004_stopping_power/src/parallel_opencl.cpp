...
static inline double stopping_power(
    ...
    const auto bracket =
        0.5 * host_compatible_log(log_argument) - beta2 - 0.5 * density_effect_delta;
...
auto main(int argc, char** argv) -> int {
    try {
        ...
        velocity_array_device =
            clCreateBuffer(context, CL_MEM_READ_ONLY, n * sizeof(double), nullptr, &status);
        opencl_check(status, "clCreateBuffer(velocity_array_device) failed.");

        stopping_power_values_device =
            clCreateBuffer(context, CL_MEM_WRITE_ONLY, n * sizeof(double), nullptr, &status);
        opencl_check(status, "clCreateBuffer(stopping_power_values_device) failed.");

        opencl_check(
            clEnqueueWriteBuffer(queue, velocity_array_device, CL_TRUE, 0, n * sizeof(double),
        ...
        opencl_check(
            clEnqueueReadBuffer(queue, stopping_power_values_device, CL_TRUE, 0, n * sizeof(double),
