...
void validate_sizes(
    ...
    const std::vector<double>& numbers_c)
{
...
sycl::event parallel_add(
    std::size_t n,
    sycl::queue& q,
    const float* sycldev_numbers_a,
    const float* sycldev_numbers_b,
    ...
    float* sycldev_numbers_c)
{
    return q.parallel_for(
        ...
        {
            const auto idx {std::size_t(i[0])};
            const auto denom {sycl::fmax(sycldev_numbers_b[idx], MIN_DENOMINATOR_32BIT)};
...
int main(int argc, char** argv)
{
    try
    {
        ...
        std::vector<float> numbers_c_32bit(n);    

        // Allocate device memory once (floats)
        float* sycldev_numbers_a = sycl::malloc_device<float>(n, q);
        float* sycldev_numbers_b = sycl::malloc_device<float>(n, q);
        float* sycldev_numbers_c = sycl::malloc_device<float>(n, q); // <-- float

        // Copy floats once
        q.memcpy(sycldev_numbers_a, numbers_a_32bit.data(), n * sizeof(float)).wait();
        q.memcpy(sycldev_numbers_b, numbers_b_32bit.data(), n * sizeof(float)).wait();
        ...
        // Copy results back and convert back to 64 bit
        q.memcpy(numbers_c_32bit.data(), sycldev_numbers_c, n * sizeof(float)).wait();
        ...
        const auto method {std::string("Parallel SYCL 32bit")};
        const auto comments {std::string("operation:") + std::string(operation_string)};

        const std::string base_file_name = "results/sycl_32bit_" + std::string(operation_string);
