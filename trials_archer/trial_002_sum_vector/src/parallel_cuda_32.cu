...
/* -------------------------------------------------------------
   ...
   ------------------------------------------------------------- */
__global__ void reduce_one_block(const float* __restrict__ in, float* __restrict__ out, int N) {
...
float task(const float* d_input, int N) {
    ...
    if (num_blocks == 1) {
        reduce_one_block<<<1, block_size, block_size * sizeof(float)>>>(d_input, d_result, N);
        CUDA_CHECK(cudaGetLastError());
        CUDA_CHECK(cudaDeviceSynchronize());

        float host_result = 0.0f;
        CUDA_CHECK(cudaMemcpy(&host_result, d_result, sizeof(float), cudaMemcpyDeviceToHost));
    ...
    reduce_one_block<<<num_blocks, block_size, block_size * sizeof(float)>>>(d_input, d_block_sums,
                                                                             N);
    CUDA_CHECK(cudaGetLastError());
    ...
    if (num_blocks <= block_size) {
        ...
        reduce_one_block<<<1, block_size, block_size * sizeof(float)>>>(d_block_sums, d_result,
                                                                        num_blocks);
    } else {
        ...
        while (remaining > 1) {
            ...
            reduce_one_block<<<blocks, block_size, block_size * sizeof(float)>>>(d_src, d_temp,
                                                                                 remaining);
            CUDA_CHECK(cudaGetLastError());
            CUDA_CHECK(cudaDeviceSynchronize());
...
int main(int argc, char** argv) {
    ...
    CUDA_CHECK(
        cudaMemcpy(device_numbers, numbers.data(), N * sizeof(float), cudaMemcpyHostToDevice));
