...
private:
    ...
    )
    {
        ...
        // Load NX and NY from SYCL engine
        const size_t NX = *cuda_engine.d_nx;
        const size_t NY = *cuda_engine.d_ny;
        ...
        while (t < t_final - 1e-15) 
        {
            ...
            for (size_t j = 1; j < model_grid.ny - 1; j++) 
            {
                for (size_t i = 1; i < model_grid.nx - 1; i++) 
                {
                    // Set x
