...
int main(int argc, char** argv) {
    ...
    // Host data generation (unchanged)
    ...
    if (!numPlatforms) { std::cerr << "No OpenCL platforms found\n"; return 2; }
    ...
    // --- Benchmark loop (replaces do { sum=task(numbers); } while ...) ---
    ...
    do {
        ...
        const cl_int zero = 0;
        check(clEnqueueWriteBuffer(queue, d_sum, CL_FALSE, 0, sizeof(cl_int), &zero, 0, nullptr, nullptr),
        ...
        check(clEnqueueReadBuffer(queue, d_sum, CL_TRUE, 0, sizeof(cl_int), &sum, 0, nullptr, nullptr),
              "clEnqueueReadBuffer(d_sum)");

        ++iters;
    } while (std::chrono::steady_clock::now() < deadline);

    std::cout << "OpenCL1," << iters << "," << sum << std::endl;
