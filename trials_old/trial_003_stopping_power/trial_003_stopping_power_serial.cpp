...
double stopping_power_Fe(double v_m_per_s) 
{
    ...
    // 0.5 * ln( (2 me c^2 beta^2 gamma^2 * Tmax) / I^2 )
    ...
    const double bracket = 0.5 * std::log(logArg) - beta2; // (no density/shell corrections)
...
void task(std::vector<double>& velocity_array, std::vector<double>& results)
{
...
{   
    ...
    for (int i = 0; i < N; ++i) 
    {
        double n = fmod(1.23456789 * 8039.0 * (last + i + 550607.0), 10000.0);
    ...
    do 
    {
        task(velocity_array, results);
        iters++;
    } 
    while (std::chrono::steady_clock::now() < deadline);
   
    std::cout << iters << "," << results[0] << "," << results[N] << std::endl;
