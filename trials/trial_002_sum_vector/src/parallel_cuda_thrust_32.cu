...
float cuda_thrust_task(const float* d_input, int N) {
    thrust::device_ptr<const float> begin(d_input);
    thrust::device_ptr<const float> end = begin + N;

    return thrust::reduce(thrust::device, begin, end, 0.0, thrust::plus<float>());
}
...
int main(int argc, char** argv) {
    ...
    do {
        calculated_value = cuda_thrust_task(device_numbers, N);
        iters++;
    ...
    // Output
    {
        const auto method{std::string("Parallel CUDA Thrust 32")};
