...
float task(const std::vector<float>& numbers) {
    const auto result = std::reduce(std::execution::par, numbers.begin(), numbers.end(), 0.0f);
    return result;
}
...
int main(int argc, char** argv) {
    ...
    // Output
    {
        const auto method{std::string("Parallel STL Reduce 32")};
        const auto operation_string = std::string("sum");
        const auto comments{std::string("operation:") + std::string(operation_string)};

        const std::string base_file_name = "results/parallel_stl_reduce_32_" + operation_string;
