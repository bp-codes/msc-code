...
double serial_simd_task_avx2(const std::vector<double>& numbers)
{
    const double* p = numbers.data();
    ...
    __m128d sum2 = _mm_add_pd(lo, hi);              // 2 doubles
    __m128d swapped = _mm_shuffle_pd(sum2, sum2, 1);
    __m128d sum1 = _mm_add_sd(sum2, swapped);
...
int main(int argc, char** argv) 
{
    ...
    do 
    {
        calculated_value = serial_simd_task_avx2(numbers);
        iters++;
    } 
    ...
    // Output
    {
        ...
        const std::string base_file_name = "results/serial_simd_" + operation_string;
