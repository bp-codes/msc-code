...
def load_precise_data(
    ...
    print("loading precise values")

    if selected_operation == "":
        raise ValueError("selected_operation must not be empty")

    # file_name = f"precise_{selected_operation}.json"
    file_name = f"precise.json"
...
def load_results(
    ...
    for json_file in sorted(results_dir.glob("*.json")):
        ...
        if json_file.name.startswith("precise"):
            continue
        ...
        for field in [
            ...
            try:
                ...
                if field == "max_rss_kb":
                    grouped[method]["max_rss_kb"].append(parsed)
            ...
            try:
                ...
                if len(method_values) != len(precise_values):
                    ...
                    array_differences = array_differences[
                        np.isfinite(array_differences)
                    ]

                    if "histogram_bins" not in grouped[method]:

                        # ----------------------------------------
                        # Compute sensible histogram range
                        # ----------------------------------------

                        if array_differences.size > 0:
                        ...
                        grouped[method]["histogram_bins"] = np.linspace(
                            -limit,
                            limit,
                            201
                        )

                        grouped[method]["histogram_counts"] = np.zeros(
                            len(grouped[method]["histogram_bins"]) - 1,
                            dtype=np.int64
                        )
...
def plot_difference_histograms(
    ...
    for method, metrics in sorted(grouped.items()):
        ...
        colour, hatch, line_colour = plot_style(method)
...
def plot_performance(
    ...
    for method, metrics in sorted(grouped.items()):
        ...
        if not max_rss_kb:
            continue
        mem_methods.append(method)
        mem_max.append(max(max_rss_kb))
