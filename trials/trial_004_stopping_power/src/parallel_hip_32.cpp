 ...
 */
__host__ __device__ static inline float hip_compatible_max(const float a, const float b) {
    return (a > b) ? a : b;
}
 ...
 */
__host__ __device__ static inline float hip_compatible_clamp(const float x, const float lo,
                                                             const float hi) {
 ...
 */
__host__ __device__ static inline float hip_compatible_log(const float x) {
 ...
 */
__host__ __device__ static inline float hip_compatible_sqrt(const float x) {
#ifdef __HIP_DEVICE_COMPILE__
    return ::sqrt(x);
#else
    return std::sqrt(x);
 ...
 * physics.
 */
__host__ __device__ static inline float stopping_power(
    ...
    const auto beta{hip_compatible_clamp(
        beta_raw, SMALL_VALUE, 0.99999f)};  // Clamped to sensible values to avoid errors
    const auto beta2{beta * beta};

    const auto inv_one_minus_beta2{1.0f / (1.0f - beta2)};
    const auto gamma2{hip_compatible_max(0.0f, inv_one_minus_beta2)};
    const auto gamma{hip_compatible_sqrt(gamma2)};

    // Total energy E = gamma * M c^2 [MeV]
    const auto total_energy_mev{hip_compatible_max(0.0f, gamma * projectile_atomic_mass_mev)};
    ...
    const auto w_max_denominator =
        hip_compatible_max(1.0f + 2.0f * gamma * electron_to_projectile_mass +
    ...
    const auto bracket =
        0.5f * hip_compatible_log(log_argument) - beta2 - 0.5f * density_effect_delta;
 ...
 * @param stopping_power_device Device pointer to outputs (length n).
 */
static inline void task(const std::size_t n, const float* const velocity_device,
                        float* const stopping_power_device) {
...
auto main(int argc, char** argv) -> int {
    ...
    auto velocity_host{std::vector<float>(n)};
    ...
    try {
        check_hip(hipMalloc(reinterpret_cast<void**>(&velocity_device), sizeof(float) * n),
                  "Failed to allocate velocity_device");
        check_hip(hipMalloc(reinterpret_cast<void**>(&stopping_power_device), sizeof(float) * n),
    ...
    check_hip(hipMemcpy(velocity_device, velocity_host.data(), sizeof(float) * n,
    ...
    check_hip(hipMemcpy(stopping_power_values.data(), stopping_power_device, sizeof(float) * n,
    ...
    const auto method{std::string("Parallel HIP 32")};
    const auto comments{std::string("stopping_power")};

    // Output
    {
        const std::string base_file_name = "results/parallel_hip_32";
