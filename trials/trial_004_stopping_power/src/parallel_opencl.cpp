 ...
 */
[[nodiscard]]
static inline double host_compatible_log(const double x)
{
    return std::log(x);
}
 ...
 */
[[nodiscard]]
static inline double host_compatible_sqrt(const double x)
{
    return std::sqrt(x);
}
...
{
    static constexpr auto SPEED_OF_LIGHT_MS {299792458.0};
    static constexpr auto ELECTRON_MASS_MEV {0.51099895000};
    static constexpr auto BETHE_CONSTANT_K  {0.307075};
    static constexpr auto SMALL_VALUE       {1.0e-9};

    const auto beta_raw {projectile_velocity_ms / SPEED_OF_LIGHT_MS};
    const auto beta {std::clamp(beta_raw, SMALL_VALUE, 0.99999)};
    ...
    const auto electron_to_projectile_mass
    {
        ELECTRON_MASS_MEV / std::max(SMALL_VALUE, projectile_atomic_mass_mev)
    };
    ...
    const auto bracket =
        0.5 * host_compatible_log(log_argument)
      - beta2
    ...
    const auto z_over_a
    {
        static_cast<double>(target_atomic_number)
...
inline double stopping_power_kernel(
    const double projectile_velocity_ms,
    const int projectile_atomic_number,
    const double projectile_atomic_mass_mev,
    const int target_atomic_number,
    ...
    const double density_effect_delta)
{
    ...
    const double ELECTRON_MASS_MEV = 0.51099895000;
    const double BETHE_CONSTANT_K  = 0.307075;
    const double SMALL_VALUE       = 1.0e-9;

    const double beta_raw = projectile_velocity_ms / SPEED_OF_LIGHT_MS;
    const double beta = clamp_double(beta_raw, SMALL_VALUE, 0.99999);
    const double beta2 = beta * beta;

    const double inv_one_minus_beta2 = 1.0 / (1.0 - beta2);
    const double gamma2 = fmax(0.0, inv_one_minus_beta2);
    const double gamma = sqrt(gamma2);

    const double electron_to_projectile_mass =
    ...
    const double w_max_denominator = fmax(
        1.0
      + 2.0 * gamma * electron_to_projectile_mass
      + (electron_to_projectile_mass * electron_to_projectile_mass),
        SMALL_VALUE);
    ...
    const double log_argument = fmax(
        (2.0 * ELECTRON_MASS_MEV * beta2 * gamma2 * w_max_mev)
      / fmax(SMALL_VALUE, mean_excitation_energy2_mev2),
        SMALL_VALUE);

    const double bracket =
        0.5 * log(log_argument)
      - beta2
      - 0.5 * density_effect_delta;

    const double projectile_charge = (double)projectile_atomic_number;
    const double projectile_charge2 = projectile_charge * projectile_charge;

    const double z_over_a =
        (double)target_atomic_number / fmax(SMALL_VALUE, target_atomic_mass_g_mol);

    const double prefactor_mass =
        BETHE_CONSTANT_K * projectile_charge2 * z_over_a / beta2;
...
__kernel void stopping_power_kernel_main(
    __global const double* velocity_device,
    __global double* stopping_power_values_device,
    const ulong n)
{
    ...
    const double PROJECTILE_ATOMIC_MASS_MEV = 938.2720813;

    const int TARGET_ATOMIC_NUMBER = 26;
    const double TARGET_ATOMIC_MASS_G_MOL = 55.845;
    const double TARGET_DENSITY_G_CM3 = 7.874;

    const double MEAN_EXCITATION_ENERGY_MEV = 286.0e-6;
    const double DENSITY_EFFECT_DELTA = 0.0;
...
{
    std::size_t size {0};
    opencl_check(clGetPlatformInfo(platform, param, 0, nullptr, &size), "clGetPlatformInfo(size) failed.");

    std::string value(size, '\0');
    opencl_check(clGetPlatformInfo(platform, param, size, value.data(), nullptr), "clGetPlatformInfo(data) failed.");
...
{
    std::size_t size {0};
    opencl_check(clGetDeviceInfo(device, param, 0, nullptr, &size), "clGetDeviceInfo(size) failed.");

    std::string value(size, '\0');
    opencl_check(clGetDeviceInfo(device, param, size, value.data(), nullptr), "clGetDeviceInfo(data) failed.");
...
cl_device_id pick_device(std::string_view device_string)
{
    ...
    for (const auto platform : platforms)
    {
        ...
        const auto device_status = clGetDeviceIDs(platform, requested_type, 0, nullptr, &device_count);
...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        velocity_array_device = clCreateBuffer(
            context,
            CL_MEM_READ_ONLY,
            n * sizeof(double),
            nullptr,
        ...
        stopping_power_values_device = clCreateBuffer(
            context,
            CL_MEM_WRITE_ONLY,
            n * sizeof(double),
            nullptr,
        ...
        opencl_check(
            clSetKernelArg(kernel, 0, sizeof(cl_mem), &velocity_array_device),
            "clSetKernelArg(0) failed.");

        opencl_check(
            clSetKernelArg(kernel, 1, sizeof(cl_mem), &stopping_power_values_device),
            "clSetKernelArg(1) failed.");

        opencl_check(
            clSetKernelArg(kernel, 2, sizeof(cl_ulong), &n_opencl),
        ...
        opencl_check(clEnqueueReadBuffer(queue, stopping_power_values_device, CL_TRUE, 0, n * sizeof(double), stopping_power_values.data(), 0, nullptr, nullptr), "clEnqueueReadBuffer(dev_c) failed.");

        const auto calculated_value {helper::check_sum(stopping_power_values)};

        // Free device allocations
        opencl_check(clReleaseMemObject(stopping_power_values_device), "clReleaseMemObject(dev_a) failed.");
        opencl_check(clReleaseMemObject(velocity_array_device), "clReleaseMemObject(dev_b) failed.");
        ...
        {
            const std::string base_file_name = "results/parallel_opencl";
