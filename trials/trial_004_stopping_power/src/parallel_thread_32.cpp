...
static inline float stopping_power(
    ...
    const auto bracket = 0.5f * std::log(log_argument) - beta2 - 0.5f * density_effect_delta;
    ...
    for (std::size_t t = 0; t < num_threads; ++t) {
        ...
        threads.emplace_back([&, begin, end]() {
            for (std::size_t i = begin; i < end; ++i) {
                results[i] = stopping_power(velocity_array[i], PROJECTILE_ATOMIC_NUMBER,
...
auto main(int argc, char** argv) -> int {
    ...
    // Output
    {
        const std::string base_file_name = "results/parallel_thread_32";
