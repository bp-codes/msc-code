...
static inline float stopping_power(
    ...
    const auto bracket =
        0.5f * sycl_compatible_log(log_argument) - beta2 - 0.5f * density_effect_delta;
...
auto main(int argc, char** argv) -> int {
    ...
    sycl::queue queue = (device_string == "CPU") ? sycl::queue{sycl::cpu_selector_v}
    ...
    if ((velocity_host == nullptr) || (stopping_power_host == nullptr) ||
        (velocity_device == nullptr) || (stopping_power_device == nullptr)) {
        ...
        if (stopping_power_host != nullptr)
