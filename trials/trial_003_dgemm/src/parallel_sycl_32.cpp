...
int main(int argc, char** argv) 
{
    ...
    Matrix<float>X_expected = dgemm_serial(k, A, B, l, C);
    float expected_value = helper::check_sum(X_expected.vector());
    ...
    do 
    {
        sgemm_sycl_tiled(q, k, dA, dB, l, dC, M, N, K, dX);
        iters++;
    } 
    ...
    float calculated_value = helper::check_sum(X.vector());
    ...
    // Output
    {
        ...
        const std::string base_file_name = "results/parallel_sycl_32_" + std::string(operation_string);
