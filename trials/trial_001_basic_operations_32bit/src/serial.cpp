...
inline constexpr float MIN_DENOMINATOR_32BIT {1.0e-9};
    ...
    std::vector<float>& numbers_c)
{
    ...
    for (auto i = std::size_t(0); i < n; i++)
    {
        numbers_c[i] = numbers_a[i] / std::max(numbers_b[i], MIN_DENOMINATOR_32BIT);
    }
}
    ...
    std::vector<float>& numbers_c)
{
    switch (operation)
    {
        case OperationKind::Add:
        {
            serial_add_32bit(numbers_a, numbers_b, numbers_c);
            return;
        }
        case OperationKind::Multiply:
        {
            serial_multiply_32bit(numbers_a, numbers_b, numbers_c);
            return;
        }
        case OperationKind::Divide:
        {
            serial_divide_32bit(numbers_a, numbers_b, numbers_c);
            return;
        }
        case OperationKind::Power:
        {
            serial_power_32bit(numbers_a, numbers_b, numbers_c);
            return;
        }
        case OperationKind::Exp:
        {
            serial_exp_32bit(numbers_a, numbers_b, numbers_c);
            return;
        }
        case OperationKind::Log:
        {
            serial_log_32bit(numbers_a, numbers_b, numbers_c);
            return;
        }
        case OperationKind::Sqrt:
        {
            serial_sqrt_32bit(numbers_a, numbers_b, numbers_c);
            return;
        }
    }
...
int main(int argc, char** argv)
{
    try
    {
        ...
        std::vector<float> numbers_a_32bit(numbers_a.begin(), numbers_a.end());
        std::vector<float> numbers_b_32bit(numbers_b.begin(), numbers_b.end());

        const auto t1 {std::chrono::steady_clock::now()};
        const auto deadline {t1 + std::chrono::duration<double>(test_time_seconds)};

        auto iters {std::uint64_t(0)};
        auto numbers_c_32bit {std::vector<float>(n)};
        //validate_sizes(numbers_a, numbers_b, numbers_c);

        do
        {
            serial_task_32bit(operation, numbers_a_32bit, numbers_b_32bit, numbers_c_32bit);
            iters++;
        }
        ...
        std::vector<double> numbers_c(numbers_c_32bit.begin(), numbers_c_32bit.end());
        ...
        const auto method {std::string("Serial 32bit")};
        ...
        out
            ...
            "time_total,max_rss_kb,passed_check,comments\n";
