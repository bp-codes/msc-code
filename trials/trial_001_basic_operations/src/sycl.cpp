 ...
 */
sycl::event parallel_add(std::size_t n, sycl::queue& q, const double* sycldev_numbers_a,
                         const double* sycldev_numbers_b, double* sycldev_numbers_c) {
 ...
 * @brief SYCL: c[i] = a[i] * b[i]
 */
sycl::event parallel_multiply(std::size_t n, sycl::queue& q, const double* sycldev_numbers_a,
                              const double* sycldev_numbers_b, double* sycldev_numbers_c) {
    return q.parallel_for(sycl::range<1>(n), [=](sycl::id<1> i) {
        const auto idx{std::size_t(i[0])};
        sycldev_numbers_c[idx] = sycldev_numbers_a[idx] * sycldev_numbers_b[idx];
 ...
 */
sycl::event parallel_divide(std::size_t n, sycl::queue& q, const double* sycldev_numbers_a,
                            const double* sycldev_numbers_b, double* sycldev_numbers_c) {
    return q.parallel_for(sycl::range<1>(n), [=](sycl::id<1> i) {
        const auto idx{std::size_t(i[0])};
 ...
 */
sycl::event parallel_power(std::size_t n, sycl::queue& q, const double* sycldev_numbers_a,
                           const double* sycldev_numbers_b, double* sycldev_numbers_c) {
    return q.parallel_for(sycl::range<1>(n), [=](sycl::id<1> i) {
        const auto idx{std::size_t(i[0])};
 ...
 */
sycl::event parallel_exp(std::size_t n, sycl::queue& q, const double* sycldev_numbers_a,
                         const double* sycldev_numbers_b, double* sycldev_numbers_c) {
    return q.parallel_for(sycl::range<1>(n), [=](sycl::id<1> i) {
        const auto idx{std::size_t(i[0])};
        sycldev_numbers_c[idx] =
            sycl::exp(sycldev_numbers_a[idx]) + sycl::exp(sycldev_numbers_b[idx]);
    });
}
 ...
 * @warning Inputs must be > 0. No bounds/validity checking is performed in this hot loop.
 */
sycl::event parallel_log(std::size_t n, sycl::queue& q, const double* sycldev_numbers_a,
                         const double* sycldev_numbers_b, double* sycldev_numbers_c) {
    return q.parallel_for(sycl::range<1>(n), [=](sycl::id<1> i) {
        const auto idx{std::size_t(i[0])};
        sycldev_numbers_c[idx] =
            sycl::log(sycldev_numbers_a[idx]) + sycl::log(sycldev_numbers_b[idx]);
    });
}
 ...
 * @warning Inputs must be >= 0. No bounds/validity checking is performed in this hot loop.
 */
sycl::event parallel_sqrt(std::size_t n, sycl::queue& q, const double* sycldev_numbers_a,
                          const double* sycldev_numbers_b, double* sycldev_numbers_c) {
    return q.parallel_for(sycl::range<1>(n), [=](sycl::id<1> i) {
        const auto idx{std::size_t(i[0])};
        sycldev_numbers_c[idx] =
            sycl::sqrt(sycldev_numbers_a[idx]) + sycl::sqrt(sycldev_numbers_b[idx]);
    });
}
...
sycl::event parallel_task(OperationKind operation, std::size_t n, sycl::queue& q,
                          const double* sycldev_numbers_a, const double* sycldev_numbers_b,
...
int main(int argc, char** argv) {
    try {
        ...
        const auto method{std::string("Parallel SYCL ") + std::string(device_string)};
        const auto comments{std::string("operation:") + std::string(operation_string)};

        // Output
        {
            const std::string base_file_name =
                "results/parallel_sycl_" + std::string(operation_string);
