    ...
    std::vector<float>& numbers_c)
{
    ...
    for (; i + 8 <= n; i += 8)
    {
        const auto va {_mm256_loadu_ps(&numbers_a[i])};
        const auto vb {_mm256_loadu_ps(&numbers_b[i])};
        const auto vc {_mm256_add_ps(va, vb)};
        _mm256_storeu_ps(&numbers_c[i], vc);
    }
    ...
    std::vector<float>& numbers_c)
{
    ...
    for (; i + 8 <= n; i += 8)
    {
        const auto va {_mm256_loadu_ps(&numbers_a[i])};
        const auto vb {_mm256_loadu_ps(&numbers_b[i])};
        const auto vc {_mm256_mul_ps(va, vb)};
        _mm256_storeu_ps(&numbers_c[i], vc);
    }
    ...
    std::vector<float>& numbers_c)
{
    ...
    const auto epsilon {_mm256_set1_ps(MIN_DENOMINATOR)};

    for (; i + 8 <= n; i += 8)
    {
        const auto va {_mm256_loadu_ps(&numbers_a[i])};
        auto vb {_mm256_loadu_ps(&numbers_b[i])};

        vb = _mm256_max_ps(vb, epsilon);

        const auto vc {_mm256_div_ps(va, vb)};
        _mm256_storeu_ps(&numbers_c[i], vc);
    }
    ...
    std::vector<float>& numbers_c)
{
    ...
    for (; i + 8 <= n; i += 8)
    {
        ...
        const auto sa {_mm256_sqrt_ps(va)};
        const auto sb {_mm256_sqrt_ps(vb)};
        const auto vc {_mm256_add_ps(sa, sb)};
...
int main(int argc, char** argv)
{
    try
    {
        ...
        // Output
        {
            ...
            const std::string base_file_name = "results/serial_simd_32_" + std::string(operation_string);
