 ...
 */
__global__ void hip_add(std::size_t n, const float* hipdev_numbers_a,
                        const float* hipdev_numbers_b, float* hipdev_numbers_c) {
 ...
 * @brief HIP: c[i] = a[i] * b[i]
 */
__global__ void hip_multiply(std::size_t n, const float* hipdev_numbers_a,
                             const float* hipdev_numbers_b, float* hipdev_numbers_c) {
    const auto idx{static_cast<std::size_t>(blockIdx.x) * blockDim.x + threadIdx.x};
 ...
 */
__global__ void hip_divide(std::size_t n, const float* hipdev_numbers_a,
                           const float* hipdev_numbers_b, float* hipdev_numbers_c) {
    const auto idx{static_cast<std::size_t>(blockIdx.x) * blockDim.x + threadIdx.x};
 ...
 */
__global__ void hip_power(std::size_t n, const float* hipdev_numbers_a,
                          const float* hipdev_numbers_b, float* hipdev_numbers_c) {
    const auto idx{static_cast<std::size_t>(blockIdx.x) * blockDim.x + threadIdx.x};
 ...
 */
__global__ void hip_exp(std::size_t n, const float* hipdev_numbers_a,
                        const float* hipdev_numbers_b, float* hipdev_numbers_c) {
    const auto idx{static_cast<std::size_t>(blockIdx.x) * blockDim.x + threadIdx.x};
 ...
 */
__global__ void hip_log(std::size_t n, const float* hipdev_numbers_a,
                        const float* hipdev_numbers_b, float* hipdev_numbers_c) {
    const auto idx{static_cast<std::size_t>(blockIdx.x) * blockDim.x + threadIdx.x};
 ...
 * @warning Inputs must be >= 0. No bounds/validity checking is performed in this hot loop.
 */
__global__ void hip_sqrt(std::size_t n, const float* hipdev_numbers_a,
                         const float* hipdev_numbers_b, float* hipdev_numbers_c) {
    const auto idx{static_cast<std::size_t>(blockIdx.x) * blockDim.x + threadIdx.x};
 ...
 */
void parallel_task(OperationKind operation, std::size_t n, const float* hipdev_numbers_a,
                   const float* hipdev_numbers_b, float* hipdev_numbers_c) {
...
auto main(int argc, char** argv) -> int {
    try {
        ...
        float* hipdev_numbers_a{};
        float* hipdev_numbers_b{};
        float* hipdev_numbers_c{};

        HIP_CHECK(hipMalloc(&hipdev_numbers_a, n * sizeof(float)));
        HIP_CHECK(hipMalloc(&hipdev_numbers_b, n * sizeof(float)));
        HIP_CHECK(hipMalloc(&hipdev_numbers_c, n * sizeof(float)));

        // Copy once
        HIP_CHECK(hipMemcpy(hipdev_numbers_a, numbers_a.data(), n * sizeof(float),
                            hipMemcpyHostToDevice));
        HIP_CHECK(hipMemcpy(hipdev_numbers_b, numbers_b.data(), n * sizeof(float),
                            hipMemcpyHostToDevice));
        ...
        // Copy results back
        HIP_CHECK(hipMemcpy(numbers_c.data(), hipdev_numbers_c, n * sizeof(float),
        ...
        const auto method{std::string("Parallel HIP 32 ") + std::string(device_string)};
        const auto comments{std::string("operation:") + std::string(operation_string)};

        // Output
        {
            const std::string base_file_name =
                "results/parallel_hip_32_" + std::string(operation_string);
