...
__global__ void sum_kernel(const int* numbers, int* result, int N) {
    ...
    unsigned int i =
        blockIdx.x * blockDim.x + threadIdx.x;  // blockIdx.x block id, blockDim.x block size (256)
...
int task(const int* device_numbers, int* device_result, int N) {
    ...
    sum_kernel<<<grid_size, block_size, block_shared_memory_size>>>(device_numbers, device_result,
                                                                    N);
...
int main(int argc, char** argv) {
    ...
    CUDA_CHECK(cudaMemcpy(device_numbers, numbers.data(), N * sizeof(int), cudaMemcpyHostToDevice));
