    ...
    __global double* X)
{
    ...
    __local double As[TILE][TILE];
    __local double Bs[TILE][TILE];
...
void opencl_setup(OpenCLContext& ctx, const Matrix<double>& A, const Matrix<double>& B,
                  const Matrix<double>& C, const Matrix<double>& X,
    ...
    auto* a_ptr = const_cast<double*>(A.data());
    ctx.matrix_a = clCreateBuffer(ctx.context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                  sizeof(double) * M * K, reinterpret_cast<void*>(a_ptr), &err);
    opencl_check(err, "buffer A failed");

    auto* b_ptr = const_cast<double*>(B.data());
    ctx.matrix_b = clCreateBuffer(ctx.context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                  sizeof(double) * K * N, reinterpret_cast<void*>(b_ptr), &err);
    opencl_check(err, "buffer B failed");

    auto* c_ptr = const_cast<double*>(C.data());
    ctx.matrix_c = clCreateBuffer(ctx.context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                  sizeof(double) * M * N, reinterpret_cast<void*>(c_ptr), &err);
    opencl_check(err, "buffer C failed");

    ctx.matrix_x =
        clCreateBuffer(ctx.context, CL_MEM_WRITE_ONLY, sizeof(double) * M * N, nullptr, &err);
    opencl_check(err, "buffer X failed");
}

void task(OpenCLContext& ctx, double alpha, const Matrix<double>& A, const Matrix<double>& B,
          double beta, const Matrix<double>& C, Matrix<double>& X) {
    cl_int err;
    ...
    opencl_check(clSetKernelArg(ctx.kernel, 3, sizeof(double), &alpha), "arg3");
...
int main(int argc, char** argv) {
    ...
    // Read back result
    opencl_check(clEnqueueReadBuffer(ctx.queue, ctx.matrix_x, CL_TRUE, 0, sizeof(double) * M * N,
    ...
    // Output
    {
        const std::string base_file_name =
            "results/parallel_opencl_" + matrix_size + "_" + std::string(operation_string);
