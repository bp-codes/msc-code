            ...
            for (std::size_t jb = 0; jb < N; jb += BLOCK) {
                ...
                for (std::size_t i = ib; i < i_max; ++i) {
                    ...
                    for (std::size_t k = kb; k < k_max; ++k) {
                        const auto kN = k * N;
                        const double a_ik = alpha * A_data[i * K + k];

                        const double* Bk = &B_data[kN];
                        double* Xi = &X_data[iN];
...
int main(int argc, char** argv) {
    ...
    const auto method{std::string("Serial Optimized")};
    const auto comments{std::string("operation:") + std::string(operation_string)};

    // Output
    {
        const std::string base_file_name =
            "results/serial_optimized_" + matrix_size + "_" + std::string(operation_string);
