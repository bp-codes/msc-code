...
void task(sycl::queue& q, float alpha, const float* A, const float* B, float beta, const float* C,
          std::size_t M, std::size_t N, std::size_t K, float* X) {
    q.submit([&](sycl::handler& h) {
         sycl::local_accessor<float, 2> tileA({TILE, TILE}, h);
         sycl::local_accessor<float, 2> tileB({TILE, TILE}, h);

         h.parallel_for(sycl::nd_range<2>(sycl::range<2>((M + TILE - 1) / TILE * TILE,
                                                         (N + TILE - 1) / TILE * TILE),
...
int main(int argc, char** argv) {
    ...
    float* dX = sycl::malloc_device<float>(M * N, q);

    // copy once
    q.memcpy(dA, A.data(), sizeof(float) * M * K).wait();
    q.memcpy(dB, B.data(), sizeof(float) * K * N).wait();
    q.memcpy(dC, C.data(), sizeof(float) * M * N).wait();
    ...
    // Test starts
    do {
        task(q, k, dA, dB, l, dC, M, N, K, dX);
        iters++;
    } while (std::chrono::steady_clock::now() < deadline);

    // Test ends
    q.memcpy(X.data(), dX, sizeof(float) * M * N).wait();
    ...
    const auto method{std::string("Parallel Sycl 32 " + matrix_size)};
    const auto comments{std::string("operation:") + std::string(operation_string)};

    // Output
    {
        const std::string base_file_name =
            "results/parallel_sycl_32_" + matrix_size + "_" + std::string(operation_string);
