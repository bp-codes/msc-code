...
public:
    ...
    void random_fill(std::mt19937_64& rng, std::uniform_real_distribution<double>& dist) {
        std::generate(_data.begin(), _data.end(), [&]() { return static_cast<T>(dist(rng)); });
    }

    [[nodiscard]] std::vector<double> data_d() const {
        std::vector<double> result;
        result.resize(_data.size());

        std::transform(_data.begin(), _data.end(), result.begin(),
