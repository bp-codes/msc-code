...
This coding standard promotes a consistent style of C++ throughout the codes developed in this work.
...
- **SHALL / SHALL NOT**  
  Mandatory requirement, equivalent to **MUST / MUST NOT**. 

- **SHOULD / SHOULD NOT**  
  Strong recommendation. Deviations are allowed only with a clear, documented justification.
...
## 19) Function declarations, `inline`, and `constexpr`

- MUST mark small, header-defined functions as inline (or constexpr when applicable).
- SHOULD prefer constexpr (and consteval where appropriate) for pure functions.
- SHOULD NOT add inline to functions defined only in .cpp files unless there is a measured reason.
- MUST use inline on header-defined variables with external linkage.

---

## 20) const, references, and parameter passing

- MUST pass read-only, non-trivial objects as const T&.
- MUST pass output or in-out parameters as non-const references (T&) or std::span<T>.
- SHOULD pass small, trivially copyable types by value.
- MUST NOT use const T for by-value parameters.
- SHOULD use pass-by-value then move when a function takes ownership.
- MUST prefer references for required non-null objects and std::span for array-like inputs.
- MUST mark member functions const when they do not modify observable state.
- SHOULD mark functions noexcept when they are guaranteed not to throw.

---

## 21) Return values and lifetimes

- MUST NOT return references or pointers to temporaries or objects with unclear lifetimes.
- SHOULD return by value for computed results.
- MAY return std::span or iterator pairs for non-owning views when lifetime is clearly documented.
