...
public:
    ...
    static void run(const std::filesystem::path& input_file) {
        ...
        for (auto i{std::size_t{0}}; i < configuration.get_time_steps(); i++) {
            ...
            if (i % configuration.get_xyz_every() == 0) {
                ConfigurationEngine::record_to_xyz(
                    static_cast<int>(i), std::filesystem::path("results/out.xyz"), configuration);
            }
        }
    ...
    static void load_json(const std::filesystem::path& input_file, Configuration& configuration) {
        ...
        std::array<double, 9> basis{ux[0], ux[1], ux[2], uy[0], uy[1], uy[2], uz[0], uz[1], uz[2]};

        std::vector<Atom> atoms{};
        if (crystal_structure == "fcc") {
            atoms = SimpleMD::Fcc::make("Al", static_cast<int>(n), static_cast<int>(n),
                                        static_cast<int>(n));
        } else if (crystal_structure == "bcc") {
            atoms = SimpleMD::Bcc::make("Al", static_cast<int>(n), static_cast<int>(n),
                                        static_cast<int>(n));
        } else {
