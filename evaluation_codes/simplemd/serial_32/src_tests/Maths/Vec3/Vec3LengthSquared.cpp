...
TEST(Vec3LengthSquared, ZeroVectorIsZero)
{
    ...
    const float found {v.length_squared()};
...
TEST(Vec3LengthSquared, DecimalComponents)
{
    ...
    const float found {v.length_squared()};
    const float expected {12.5}; // 1.5^2 + (-2.5)^2 + 2^2 = 2.25 + 6.25 + 4.0
    const float tolerance {1.0e-9};
    EXPECT_NEAR(found, expected, tolerance);
}
...
TEST(Vec3LengthSquared, WorksWithLargeMagnitudes)
{
    ...
    const float found {v.length_squared()};
    const float expected {3.0e300}; // 3 * (1e150^2)
    const float tolerance {1.0e291}; // relative ~1e-9
    EXPECT_NEAR(found, expected, tolerance);
}
