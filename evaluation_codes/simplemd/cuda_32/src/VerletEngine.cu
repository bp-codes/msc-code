...
__global__ void zero_forces_kernel(Atom* atoms, std::size_t n_atoms)
{
    ...
    atoms[i].force.x = 0.0f;
    atoms[i].force.y = 0.0f;
    atoms[i].force.z = 0.0f;
}
                                       ...
                                       float r_cutoff)
{
    ...
    const float force_mag = SimpleMD::Morse::force(pair.r, 0.343f, 1.44f, 2.863f);

    const float fx = force_mag * pair.u_vec.x;
    const float fy = force_mag * pair.u_vec.y;
    const float fz = force_mag * pair.u_vec.z;
                                          ...
                                          float dt)
{
    ...
    const float c = 0.5f * atom.inv_mass * dt * dt;
                                          ...
                                          float dt)
{
    ...
    const float c = 0.5f * atom.inv_mass * dt;
...
void VerletEngine::calculate_position(Configuration& configuration)
{
    ...
    const float dt = configuration._dt;
