    ...
    for (int ii = -1; ii <= 1; ++ii) {
        for (int jj = -1; jj <= 1; ++jj) {
            for (int kk = -1; kk <= 1; ++kk) {
                ...
                const float dx = atom_j.position.x + static_cast<float>(ii) - atom_i.position.x;
                const float dy = atom_j.position.y + static_cast<float>(jj) - atom_i.position.y;
                const float dz = atom_j.position.z + static_cast<float>(kk) - atom_i.position.z;
                ...
                const float r = fmax(sqrt(r_sq), 1.0e-9f);
                                             ...
                                             Basis3 basis)
{
    ...
    const float dx = atom_j.position.x + atom_pair.position_offset.x - atom_i.position.x;
    const float dy = atom_j.position.y + atom_pair.position_offset.y - atom_i.position.y;
    const float dz = atom_j.position.z + atom_pair.position_offset.z - atom_i.position.z;
    ...
    if (r > 0.0f) {
        ...
        atom_pair.u_vec.x = 0.0f;
        atom_pair.u_vec.y = 0.0f;
        atom_pair.u_vec.z = 0.0f;
    }
}
...
void ConfigurationEngine::make_neighbour_list(Configuration& configuration)
{
    ...
    const float alat = configuration._alat;
    Basis3 basis {};
    ...
    const float r_verlet_cutoff = configuration._r_verlet_cutoff;
