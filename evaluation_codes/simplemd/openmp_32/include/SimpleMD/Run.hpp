...
public:
    static void run(const std::filesystem::path& input_file) {
        ...
        for (int i = 0; i < configuration.get_time_steps(); i++) {
            ...
            if (i % configuration.get_xyz_every() == 0)
                ConfigurationEngine::record_to_xyz(i, std::filesystem::path("results/out.xyz"),
                                                   configuration);
        }
    ...
    void static load_json(const std::filesystem::path& input_file, Configuration& configuration) {
        ...
        float heat = Run::load<float>(config, {"crystal", "heat"});
        std::string crystal_structure = Run::load<std::string>(config, {"crystal", "structure"});
        float alat = Run::load<float>(config, {"crystal", "alat"});
        std::size_t n = Run::load<std::size_t>(config, {"crystal", "size"});
        std::vector<float> ux = Run::load<std::vector<float>>(config, {"crystal", "ux"});
        std::vector<float> uy = Run::load<std::vector<float>>(config, {"crystal", "uy"});
        std::vector<float> uz = Run::load<std::vector<float>>(config, {"crystal", "uz"});

        float r_cutoff = Run::load<float>(config, {"simulation", "r_cutoff"});
        float r_verlet_cutoff = Run::load<float>(config, {"simulation", "r_verlet_cutoff"});
        std::size_t rebuild_every = Run::load<std::size_t>(config, {"simulation", "rebuild_every"});
        float dt = Run::load<float>(config, {"simulation", "dt"});
