...
TEST(Vec3Distance, SameVectorIsZero) {
    ...
    const float found{a.distance(b)};
...
TEST(Vec3Distance, CubeDiagonal) {
    ...
    const float found{a.distance(b)};
    const float expected{1.732050807568877};  // sqrt(3)
    const float tolerance{1.0e-9};
    EXPECT_NEAR(found, expected, tolerance);
}
...
TEST(Vec3Distance, MixedSigns) {
    ...
    const float found{a.distance(b)};
    const float expected{6.12372435695795};  // precomputed
    const float tolerance{1.0e-9};
    EXPECT_NEAR(found, expected, tolerance);
}
...
TEST(Vec3Distance, WorksWithLargeMagnitudes) {
    ...
    const float found{a.distance(b)};
    const float expected{3.464101615137754e150};  // 2A*sqrt(3)
