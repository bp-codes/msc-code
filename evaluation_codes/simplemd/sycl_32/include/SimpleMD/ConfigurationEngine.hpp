...
public:
     ...
     */
    static inline void heat(Configuration& configuration, float T_target,
                            float kB = 8.617333262145e-5f,  // eV/K (change if you use SI)
        ...
        for (auto& atom : atoms) {
            Maths::Vec3 d_position = {sigma * static_cast<float>(normal01(rng)),
    ...
    static void make_neighbour_list(Configuration& configuration) {
        ...
        const float r_verlet_cutoff = configuration.get_r_verlet_cutoff();
        const float r_verlet_cutoff_sq = r_verlet_cutoff * r_verlet_cutoff;
...
#pragma omp critical
    ...
    static void pair_atoms(Configuration& configuration, const float r_verlet_cutoff_sq,
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                for (int k = -1; k <= 1; k++) {
                    Maths::Vec3 position_offset = {1.0f * i, 1.0f * j, 1.0f * k};
                    ...
                    if (r_sq <= r_verlet_cutoff_sq) {
                        ...
                        atom_pair.r = Maths::fmax(atom_pair.r, 1.0e-9f);

                        auto inv_r = 1.0f / atom_pair.r;
...
#pragma omp parallel for
        for (std::size_t idx = 0; idx < n_pairs; ++idx) {
            ...
            if (atom_pair.r > 0.0f) {
                atom_pair.u_vec = (1.0f / atom_pair.r) * r_vec;
            } else {
                atom_pair.u_vec = {};
            }
        }
    ...
    static void make_neighbour_list_sycl(Configuration& configuration) {
        ...
        if (configuration._d_neighbour_list_size == nullptr) {
            configuration._d_neighbour_list_size = sycl::malloc_shared<std::size_t>(1, q);
        }
        ...
        *configuration._d_neighbour_list_size = 0;
        ...
        float d_alat = configuration._alat;
        auto d_basis = configuration._basis;
        float d_r_verlet_cutoff = configuration._r_verlet_cutoff;
        float d_r_verlet_cutoff_sq = d_r_verlet_cutoff * d_r_verlet_cutoff;
        ...
        q.submit([&](sycl::handler& h) {
             h.parallel_for(sycl::range<2>(n_atoms, n_atoms), [=](sycl::id<2> idx) {
                 ...
                 for (int ii = -1; ii <= 1; ++ii) {
                     for (int jj = -1; jj <= 1; ++jj) {
                         for (int kk = -1; kk <= 1; ++kk) {
                             Maths::Vec3 position_offset = {1.0f * ii, 1.0f * jj, 1.0f * kk};

                             const auto r_vec =
                                 Maths::Vec3::separation(d_alat, d_basis, atom_i.position,
                                                         atom_j.position + position_offset);

                             const float r_sq = r_vec.length_squared();
                             if (r_sq > d_r_verlet_cutoff_sq) {
                                 continue;
                             }

                             const float r = sycl::max(sycl::sqrt(r_sq), 1.0e-9f);
                             if (r <= 0.0f) {
                                 continue;
                             }
        ...
        if (neighbour_count > 0) {
            q.memcpy(neighbour_list.data(), configuration._d_neighbour_list,
    ...
    static void update_neighbour_list_sycl(Configuration& configuration) {
        ...
        const float alat = configuration.get_alat();
        const auto basis = configuration.get_basis();  // std::array<float, 9>

        q.submit([&](sycl::handler& h) {
             h.parallel_for(sycl::range<1>(n_pairs), [=](sycl::id<1> idx) {
                 ...
                 const auto r_vec = Maths::Vec3::separation(
                     alat, basis, atom_i.position, atom_j.position + atom_pair.position_offset);

                 const float r_sq = r_vec.length_squared();
                 const float r = Maths::sqrt(r_sq);

                 atom_pair.r = r;

                 if (r > 0.0f) {
                     atom_pair.u_vec = (1.0f / r) * r_vec;
                 } else {
