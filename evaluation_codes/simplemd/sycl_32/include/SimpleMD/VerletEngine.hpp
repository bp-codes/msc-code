...
public:
    ...
    static void calculate_force_sycl(Configuration& configuration) {
        ...
        const float r_cutoff = configuration._r_cutoff;
        ...
        q.submit([&](sycl::handler& h) {
             h.parallel_for(sycl::range<1>(n_pairs), [=](sycl::id<1> idx) {
                 ...
                 const float force_mag = SimpleMD::Morse::force(pair.r, 0.343, 1.44, 2.863);
                 ...
                 using atomic_float = sycl::atomic_ref<float, sycl::memory_order::relaxed,
                                                       sycl::memory_scope::device,
                                                       sycl::access::address_space::global_space>;

                 // i atom gets -vforce
                 atomic_float(fi.x).fetch_add(-vforce.x);
                 atomic_float(fi.y).fetch_add(-vforce.y);
                 atomic_float(fi.z).fetch_add(-vforce.z);

                 // j atom gets +vforce
                 atomic_float(fj.x).fetch_add(vforce.x);
                 atomic_float(fj.y).fetch_add(vforce.y);
                 atomic_float(fj.z).fetch_add(vforce.z);
             });
         }).wait();
