...
private:
    ...
    static void read_input(std::string& input_file, Grid& model_grid, std::vector<Source>& sources,
                           double& dt, double& t_final, int& snapshot_every,
     ...
     */
    static void heat_grid(Grid& model_grid, const std::vector<Source>& sources, const double& dt,
        ...
        while (t < t_final - 1e-15) {
            ...
            for (auto j{std::size_t(1)}; j < model_grid.ny - 1; j++) {
                for (auto i{std::size_t(1)}; i < model_grid.nx - 1; i++) {
                    ...
                    const auto lap =
                        (model_grid.at(i + 1, j) - 2.0 * uij + model_grid.at(i - 1, j)) *
                            model_grid.invdx2 +
                        (model_grid.at(i, j + 1) - 2.0 * uij + model_grid.at(i, j - 1)) *
                            model_grid.invdy2;
