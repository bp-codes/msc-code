 ...
 */
__global__ void set_top_bottom(double* __restrict__ un, std::size_t NX, std::size_t NY) {
 ...
 * @param NY Grid size in y.
 */
__global__ void set_left_right(double* __restrict__ un, std::size_t NX, std::size_t NY) {
 ...
 */
__device__ inline double device_source_value_at(const Source& s, double t, double x, double y,
 ...
 */
__global__ void heat_step_kernel(const double* __restrict__ u,      // in
                                 double* __restrict__ un,           // out
                                 const double* __restrict__ alpha,  // per-cell diffusivity
                                 std::size_t nx, std::size_t ny, double invdx2, double invdy2,
                                 double dx, double dy, double dt,
                                 const Source* __restrict__ sources, std::size_t num_sources,
                                 double t_mid  // midpoint time (t + 0.5*dt)
) {
    const int i = blockIdx.x * blockDim.x + threadIdx.x;
    const int j = blockIdx.y * blockDim.y + threadIdx.y;

    if (i <= 0 || j <= 0 || i >= static_cast<int>(nx) - 1 || j >= static_cast<int>(ny) - 1)
        return;
...
void CudaEngine::_allocate() {
    ...
    CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&d_sources), num_sources * sizeof(Source)));
}
...
void CudaEngine::_cleanup() {
    ...
    if (d_num_sources) {
        cudaFree(d_num_sources);
        d_num_sources = nullptr;
    }
}

// Copy model grid from host -> device
void CudaEngine::upload_grid(const Grid& model_grid, const std::vector<Source>& sources) {
    // Validate sizes
    ...
    CUDA_CHECK(
        cudaMemcpy(d_length_x, &model_grid.length_x, sizeof(double), cudaMemcpyHostToDevice));
    CUDA_CHECK(
        cudaMemcpy(d_length_y, &model_grid.length_y, sizeof(double), cudaMemcpyHostToDevice));
    ...
    CUDA_CHECK(
        cudaMemcpy(d_num_sources, &num_sources_host, sizeof(std::size_t), cudaMemcpyHostToDevice));
    ...
    if (num_sources_host) {
        CUDA_CHECK(cudaMemcpy(d_sources, sources.data(), num_sources_host * sizeof(Source),
...
void CudaEngine::heat_step(double dt, double t_mid) {
    ...
    heat_step_kernel<<<grid, block>>>(d_u, d_un, d_alpha, nx, ny, h_invdx2, h_invdy2, h_dx, h_dy,
                                      dt, d_sources, num_sources, t_mid);
